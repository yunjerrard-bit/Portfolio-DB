...
gap-fix 01-07: 사용자 결정으로 모든 DIFF는 종가 EMA 기준으로 통일.
  ...
  - trend: EMA_Close_N.pct_change() × 4 periods = 4개 (비율) [gap-fix 01-11]

gap-fix 01-12: dailychg 컬럼 4개 완전 제거 (trend가 대체).
"""
...
def add_ema_columns(df: pd.DataFrame) -> pd.DataFrame:
    """입력 df copy + 20 신규 컬럼 (4 EMA + 12 DIFF + 4 trend).

    컬럼 명명 (gap-fix 01-07 + 01-11 + 01-12):
      ...
      (gap-fix 01-12: dailychg 컬럼은 제거됨 — trend가 대체.)
    """
    out = df.copy()
    # EMA_Close_N + trend (4 + 4)
