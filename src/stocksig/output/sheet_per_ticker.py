...
97 컬럼 (gap-fix 01-14 95 + 주봉 EMA 진행형 추세 2).
  ...
  - EMA_Close × 4 (값/med/std/trend) + 주봉 EMA 진행형 추세 11·22 (AK·AL)
  - DIFF × 12 (med/std)
...
def build_column_layout(df: pd.DataFrame | None = None) -> list[str]:
    """97 컬럼 이름 list (gap-fix 01-14 95 + 주봉 EMA 진행형 추세 2).
    ...
    구조:
      ...
      + EMA_Close × 4 (val,med,std,trend) = 16
        — 단, 첫 EMA11 그룹의 hidden 트리오(val/med/std) 다음 + 일봉 EMA11 trend 직전에
          주봉 EMA 진행형 추세 2개(11, 22)를 삽입 (AK·AL = (주)ema11·22 추세).
      + DIFF × 12 (val,med,std) = 36
      + Stoch %K/%D 일/주 = 4
      + RSI 일/주 = 2
      + MACD_OSC 일/주 = 2
      + Impulse 일/주 = 2
      = 1+9+9+3+3+2+3+3+16+2+36+4+2+2+2 = 97
    """
    ...
    # EMA_Close × 4 (16) + 주봉 EMA 진행형 추세 (11·22 = 2) — 총 18
    # 첫 EMA11 그룹 hidden 트리오 다음, 일봉 EMA11 trend 직전에 주봉 추세 2개 삽입.
    # 결과: AH=일EMA11(hidden) · AI=med(h) · AJ=std(h) · AK=(주)ema11 추세 ·
    #       AL=(주)ema22 추세 · AM=일EMA11 추세 · AN~ 일EMA22 그룹 …
    layout += ["EMA_Close_11", "EMA_Close_11_median", "EMA_Close_11_std"]
    layout += ["EMA_Close_11_week_trend", "EMA_Close_22_week_trend"]
    layout += ["EMA_Close_11_trend"]
    for n in (22, 96, 192):
        base = f"EMA_Close_{n}"
...
def korean_header(col: str) -> str:
    ...
    # EMA_Close_{n}_week_trend (주봉 진행형 추세) — 일봉 _trend 매칭보다 먼저 처리.
    if col.startswith("EMA_Close_") and col.endswith("_week_trend"):
        n = col[len("EMA_Close_") : -len("_week_trend")]
        return f"(주)ema{n} 추세"

    # EMA_Close_{n}_trend (일봉)
